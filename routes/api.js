...
module.exports = function (io) {
    ...
    router.patch('/scheduled/:id', uploadMedia.single('file'), (req, res) => {
        ...
        const { message, send_at, repeat_type, archive_path, archive_type } = req.body;
        ...
        if (req.file) {
            // Bilgisayardan yeni dosya yüklendi
            ...
            // Arşivden seçildi — path tarayıcı URL'i (/media/archive/...), sunucu yoluna çevir
            const serverPath = path.join(ROOT, archive_path);
            db.prepare('UPDATE scheduled_messages SET message = ?, send_at = ?, repeat_type = ?, media_path = ?, media_type = ? WHERE id = ?')
                .run(message ?? '', sendAtDate.toISOString(), repeat_type || 'none', serverPath, archive_type || 'image', id);
        } else {
